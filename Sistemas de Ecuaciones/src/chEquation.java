...
import com.berkeleychurchill.chembal.chChemical;
import com.berkeleychurchill.chembal.chElement;
...
public class chEquation{
        ...
        throws InvalidUserInputException
    {
        ...
        System.out.println(chEquation.matrizSToString(coeffMatrix));
        int answ[][];
        try
        {
    ...
    public static String matrizSToString(double [][] matriz){
		StringBuilder sb = new StringBuilder();
		for(int i=0; i<matriz.length; i++){
			for(int j=0; j<matriz[0].length; j++)
				sb.append(matriz[i][j]+" ");
    ...
    public static void main(String [] args) throws InvalidUserInputException{
    	chEquation eq= new chEquation("C3H8+O2->CO2+H2O");
    	System.out.println( eq.matrizToString(eq.Balance()) );
    	System.out.println( eq.getItem(3));
    }
}
